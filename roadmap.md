...
4. `308A-1` — **centralizar runtime del gate** (checkout compartido `.quality-tools/` + `sourcePathEnv` + `quality:sync`); `glory-sentinel` queda exento de gate (excepción en el panel). **AVANCE (2026-08-30):** F0/F1 ✅ checkout compartido (`sentinel@643353d` v0.7.5 + `varsense@88f281f` en `area-trabajo/.quality-tools/`, detached/limpios, ignorado por el scanner) y F6 ✅ excepción `sinGate` en workspace-manager (v3: types/configArea/scanner + endpoint `/api/config/singate` restrictivo + subsección «sin gate» en el PanelConfig; `glory-sentinel` registrado en la config, escaneo real fuerza `puerta:none` sin romper los 6 consumidores). F2–F4 (migrar los 6 consumidores a `sourcePathEnv` + regenerar locks) quedan **bloqueados por dependencia en el server** — S2-05 era **stale** y quedó resuelto: provisioné ambos checkouts compartidos (`GLORY::.quality-tools/sentinel` compilado v0.7.5 `out/cli/index.js` responde, `GLORY::.quality-tools/varsense` compilado v2.2.1 `dist/cli/index.js` responde; `npm run compile` reproducible exit 0; detached/limpios). Piloto gloryapi migrado a `sourcePathEnv` con la env → `task:check GLORY-BASELINE` PASS (0 errores); **sin la env el doctor queda BLOQUEADO (7 problemas)** porque el server del manager aún no deriva las `GLORY_*` (solo lee `WS_AREA_ROOT`/`WS_PORT`). ⇒ Paso previo de F2–F4: derivar `GLORY_SENTINEL_SOURCE_PATH`/`GLORY_VARSENSE_SOURCE_PATH` en `src/server` desde `RAÍZ_AREA/.quality-tools/{sentinel,varsense}`; piloto revertido al `sourcePath` previo para no romper el gate (gloryapi limpio). Falta F5 (`quality:sync`) y F7. Ver `PLAN-centralizar-gate.md` (reorienta el antiguo `PLAN-agregar-gate-proyectos.md`).
